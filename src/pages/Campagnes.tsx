...
const Campagnes = () => {
  ...
  const filtered = recentCampaigns.filter(
    (c) => c.nom.toLowerCase().includes(search.toLowerCase()) || c.region.toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass-card rounded-xl p-5">
        <h3 className="text-sm font-semibold text-dashboard-card-foreground mb-4" style={{ fontFamily: "Outfit" }}>Évolution Mensuelle des Campagnes</h3>
        <ResponsiveContainer width="100%" height={280}>
          <AreaChart data={monthlyParticipants}>
            ...
            <Area type="monotone" dataKey="campagnes" stroke="hsl(152, 60%, 45%)" fill="url(#gradCamp)" strokeWidth={2} />
            <Area type="monotone" dataKey="participants" stroke="hsl(152, 60%, 45%)" fill="transparent" strokeWidth={2} strokeDasharray="5 5" />
            <Legend />
          </AreaChart>
        </ResponsiveContainer>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass-card rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold text-dashboard-card-foreground" style={{ fontFamily: "Outfit" }}>Liste des Campagnes</h3>
